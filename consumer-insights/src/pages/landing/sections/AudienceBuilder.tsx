...
// Each active filter reduces the audience by a multiplier
...
function calcReach(active: Set<string>) {
  let reach = 8200000  // total population base
  active.forEach(id => {
    reach *= (FILTER_MULTIPLIERS[id] ?? 0.7)
  })
          ...
          <div className="relative z-10 p-8 flex flex-col h-full" style={{ minHeight: 480 }}>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {FILTER_OPTIONS.map(({ id, label, group }) => {
                ...
                return (
                  ...
                  >
                    <span className="text-[10px] text-current/60">{group}</span>
                    <span>{label}</span>
                    <AnimatePresence>
                      {isActive && (
                        <motion.span
            ...
            <div className="mt-auto">
              <div className="inline-flex flex-col gap-1 rounded-xl border border-border bg-background/90 backdrop-blur-sm px-5 py-4 shadow-[rgba(0,0,0,0.06)_0px_0px_16px_0px]">
                <p className="text-xs text-muted-foreground font-medium">Estimated reach</p>
                <p className="text-3xl font-semibold text-foreground">
                  <ReachNumber key={reach} value={reach} />
                  <span className="text-lg text-muted-foreground ml-1">respondents</span>
                </p>
                {active.size > 0 && (
