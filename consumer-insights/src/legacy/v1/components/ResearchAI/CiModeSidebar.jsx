...
function ModeSwitcher({ mode, onModeChange }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', background: '#eef0f3', borderRadius: 8, padding: 3, gap: 2, marginBottom: 4, flexShrink: 0 }}>
        ...
        return (
          <button
            ...
            style={{
              position: 'relative',
              flex: active ? 1 : 0,
              minWidth: active ? 0 : 32,
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5,
              height: 30, borderRadius: 6, border: 'none', cursor: 'pointer',
              padding: active ? '0 10px' : '0',
              background: active ? 'white' : 'transparent',
              boxShadow: active ? '0 1px 3px rgba(15,39,65,0.12)' : 'none',
              transition: 'background 0.15s, box-shadow 0.15s, flex 0.2s',
              fontFamily: FF, fontSize: 12, fontWeight: 600,
              color: '#0f2741', whiteSpace: 'nowrap', overflow: 'visible',
            }}
          >
            <Icon active={active} />
...
export default function CiModeSidebar({ collapsed, onToggle, mode, onModeChange, onNewChat }) {
  ...
  return (
    <div className={s.inner}>
      ...
      <a href="/consumer_insights_v1/dashboard" className={s.navItem}>
        <IconDashboard />
        Dashboard
