...
const CHART_COLORS = ['#0666e5', '#adc5ce', '#7bafd4', '#455f7c', '#c4c4c4']
...
export default function SurveyCard({ name, data, onClose, onOpenDetails, detailsOpen }) {
  ...
  const activeView = isCrosstab ? 'table' : chartView
  ...
  const allGroups = useMemo(() => {
    ...
    return [totalGroup, ...colGroups]
  }, [isCrosstab, colGroups, data, name])
                          ...
                          return activeSubCols.map((sc, sci) => {
                            ...
                            return (
                              <td
                                ...
                                style={sc.key === 'pct' && !r.isBase ? { backgroundColor: heatmapBg(pctVal), color: heatmapTextColor(pctVal) } : undefined}
                              >
                                {cell[sc.key]}
                              </td>
                            )
